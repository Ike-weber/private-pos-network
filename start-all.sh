...
# Start Geth nodes
geth --datadir node1 --port 30301 --http --http.port 8541 --http.api eth,net,engine,admin --authrpc.port 8551 --authrpc.jwtsecret jwt.hex --syncmode full --networkid 12345 --ipcdisable >> logs/geth1.log 2>&1 &
geth --datadir node2 --port 30302 --http --http.port 8542 --http.api eth,net,engine,admin --authrpc.port 8552 --authrpc.jwtsecret jwt.hex --syncmode full --networkid 12345 --ipcdisable >> logs/geth2.log 2>&1 &
geth --datadir node3 --port 30303 --http --http.port 8543 --http.api eth,net,engine,admin --authrpc.port 8553 --authrpc.jwtsecret jwt.hex --syncmode full --networkid 12345 --ipcdisable >> logs/geth3.log 2>&1 &
echo "Geth started"
sleep 10
...
# Start beacon1
./prysm.sh beacon-chain --datadir beacon1 --min-sync-peers 0 --genesis-state genesis.ssz --interop-eth1data-votes --chain-config-file config.yaml --contract-deployment-block 0 --chain-id 12345 --accept-terms-of-use --jwt-secret jwt.hex --suggested-fee-recipient 0x8B0681dBD724dcaC48b433e9df8A220D47C94a19 --execution-endpoint http://localhost:8551 --rpc-port 4000 --grpc-gateway-port 3500 --p2p-tcp-port 13000 --p2p-udp-port 12000 --bootstrap-node= >> logs/beacon1.log 2>&1 &
...
./prysm.sh beacon-chain --datadir beacon2 --min-sync-peers 0 --genesis-state genesis.ssz --interop-eth1data-votes --chain-config-file config.yaml --contract-deployment-block 0 --chain-id 12345 --accept-terms-of-use --jwt-secret jwt.hex --suggested-fee-recipient 0xC4d87b80780117F805D620c4FF88e5380699dB41 --execution-endpoint http://localhost:8552 --rpc-port 4001 --grpc-gateway-port 3501 --p2p-tcp-port 13001 --p2p-udp-port 12001 --bootstrap-node=/ip4/127.0.0.1/tcp/13000/p2p/$BEACON1_PEER >> logs/beacon2.log 2>&1 &
...
# Start validators
./prysm.sh validator --datadir beacon1/validator --accept-terms-of-use --chain-config-file config.yaml --interop-num-validators 1 --interop-start-index 0 --beacon-rest-api-provider http://localhost:3500 >> logs/validator1.log 2>&1 &
./prysm.sh validator --datadir beacon2/validator --accept-terms-of-use --chain-config-file config.yaml --interop-num-validators 1 --interop-start-index 1 --beacon-rest-api-provider http://localhost:3501 >> logs/validator2.log 2>&1 &
./prysm.sh validator --datadir beacon3/validator --accept-terms-of-use --chain-config-file config.yaml --interop-num-validators 1 --interop-start-index 2 --beacon-rest-api-provider http://localhost:3502 >> logs/validator3.log 2>&1 &
